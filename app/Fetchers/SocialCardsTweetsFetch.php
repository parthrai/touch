...
class SocialCardsTweetsFetch
{
  ...
  public function FetchTweets ()
  {
    ...
    $dirty           = false;
    $hashtag_configs = TwitterHashtagConfig::where( 'enabled', '=', 1 )->get();
    ...
    foreach( $hashtag_configs as $hashtag_config )
    {
      ...
      $team_hashtags = ScoreboardTeamConfig::GetHashtags();

      $since_id      = SocialCardTweet::orderBy( 'card_created_at', 'desc' )
      ->pluck( 'tweet_id' )
      ->first();
      ...
      if( $this->QueryTweets( '#' . $query_hashtag, $since_id ) )
      {
        $dirty = true;
      }
      ...
      foreach( $team_hashtags as $team_name => $team_hashtag )
      {
        ...
        if( $this->QueryTweets( $query, $since_id ) )
        {
          $dirty = true;
        }
   ...
   * @return Bool
   */
  private function QueryTweets ( $query, $since_id )
  {
    ...
    try
    {
      ...
      $json = Twitter::getSearch(
        [
          'q'          => $query,
          'count'      => intval( getenv( 'SOCIALCARDS_TWEETS_BATCHSIZE' ) ),
          'since_id'   => $since_id,
          'format'     => 'json',
          'tweet_mode' => 'extended'
        ]
    ...
    foreach( $items->statuses as $status )
    {
      ...
      $tweet = SocialCardTweet::where( 'tweet_id', '=', $status->id_str )->first();
